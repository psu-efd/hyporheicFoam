...
Foam::autoPtr<Foam::reactionModels::rhoReactionModel>
Foam::reactionModels::rhoReactionModel::New
(
    const fvMesh& mesh,
...
)
{
    ...
    (
        ...
        ).lookup("reactionModel")
    );

    Info<< "Selecting reaction model " << combTypeName << endl;
    ...
    return autoPtr<rhoReactionModel>
    (
        cstrIter()(className, mesh, reactionProperties, phaseName)
    );
}
